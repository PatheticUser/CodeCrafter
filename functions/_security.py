...
All file-based tool functions should use these validators instead of
duplicating the path traversal check. Using realpath resolves symlinks
...
def validate_path(working_directory: str, target_path: str) -> Optional[str]:
    ...
    Args:
        working_directory: The sandboxed working directory (absolute or relative).
        target_path: Path to validate, treated as relative to working_directory.

    Returns:
        None if the path is safe, or an error message string if it escapes.
    """
    working_dir_abs = os.path.realpath(working_directory)
    target_abs = os.path.realpath(os.path.join(working_directory, target_path))

    if not (
        target_abs == working_dir_abs
        or target_abs.startswith(working_dir_abs + os.sep)
    ):
        return (
            f'Error: Cannot access "{target_path}" — '
...
def validate_directory(working_directory: str, target_dir: str) -> Optional[str]:
    ...
    Args:
        ...
        target_dir: Directory to validate, relative to working_directory.
...
def validate_working_directory(working_directory: str) -> Optional[str]:
